...
# This for loop takes one element from the list L at a
# time and stores that in the element length.
# It does this for all elements in the list.
for length in L:
    L_meters.append(length / 100)
    # Converts the length to metres using 'length/100'.
    # Adds the metre converted length to a new list called L_meters
...
# if we say this, this means after each time the code runs, the value of i increments by 1[since increment = 1] from 1 till 6
...
# counts down from 10 till 1, as the increments are -1 or a decrement of 1. The values that i takes are 10, 9, 8, 7, 6, 5, 4, 3, 2.
...
# if two parameters are given, the first is assumed to be the starting value and the last is assumed to be the ending value. The default increment is taken to be 1.
for i in range(1, 7):
    print(i)
...
while 2 > 1:  # since the expression 2 > 1 is always true, the loop continues to run endlessly.
    print("Whee!")

# The meter conversion, but this time using a while loop
i = 0
L_m = []
# While the expression i < len(L) is true the loop runs. len(L) refers the length of the list, which is 3 in this case as it contains 3 elements.
while i < len(L):
    L_m.append(L[i] / 100)
    i += 1
